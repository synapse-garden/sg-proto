...
// Convo sets up the Convos API on the Router for the given DB.
func Convo(r *htr.Router, db *bolt.DB) error {
...
// ConnectConvo returns a Handle which opens and binds a WebSocket
// session to a Convo.  The messages are transported as-is.
func ConnectConvo(db *bolt.DB) htr.Handle {
...
func GetMessages(db *bolt.DB) htr.Handle {
	return func(w http.ResponseWriter, r *http.Request, ps htr.Params) {
...
// NewConvo returns a Handle over the DB which checks that the POSTed
// Convo is valid and then creates it, returning the created Convo.
func NewConvo(db *bolt.DB) htr.Handle {
...
// UpdateConvo returns a Handle which updates a Convo in the DB by ID.
func UpdateConvo(db *bolt.DB) htr.Handle {
	return func(w http.ResponseWriter, r *http.Request, ps htr.Params) {
		userID := mw.CtxGetUserID(r)
		id := ps.ByName("convo_id")
		if _, err := uuid.FromString(id); err != nil {
...
// GetConvo returns a Handle which gets the given Convo by ID.  Any user
// who is an Owner, Reader, or Writer can get a Convo by ID.
func GetConvo(db *bolt.DB) htr.Handle {
	return func(w http.ResponseWriter, r *http.Request, ps htr.Params) {
		userID := mw.CtxGetUserID(r)
		id := ps.ByName("convo_id")
		if _, err := uuid.FromString(id); err != nil {
...
// DeleteConvo deletes the given Convo by ID.
func DeleteConvo(db *bolt.DB) htr.Handle {
	return func(w http.ResponseWriter, r *http.Request, ps htr.Params) {
		userID := mw.CtxGetUserID(r)
		id := ps.ByName("convo_id")
		if _, err := uuid.FromString(id); err != nil {
